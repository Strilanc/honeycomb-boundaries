...
def main():
    ...
    all_data = all_data.filter(lambda e: e.data_height / e.data_width > 1.6)
    ...
    fig3.savefig(OUT_DIR / "teraquop.pdf", bbox_inches='tight')
    fig3.savefig(OUT_DIR / "teraquop.png", bbox_inches='tight', dpi=200)
...
def plot_lambda_line_fits_combo(all_data: MultiStats, focus: bool) -> Tuple[plt.Figure, plt.Axes]:
    styles = {
        "SD6": [
            ...
            ("bounded_honeycomb_memory_SD6", "internal"),
            # ("bounded_honeycomb_memory_SD6", "internal_correlated"),
        ],
        "SI1000": [
            ...
            ("bounded_honeycomb_memory_SI1000", "internal"),
            # ("bounded_honeycomb_memory_SI1000", "internal_correlated"),
        ],
        "EM3": [
            None,
            None,
            ...
            ("bounded_honeycomb_memory_EM3_v2", "internal"),
            # ("bounded_honeycomb_memory_EM3_v2", "internal_correlated"),
        ],
    }
    if focus:
        styles = {
...
class LambdaGroup:
    ...
    def combo_data(self) -> Tuple[List[int], List[float]]:
        distances = sorted(
            set(self.distance_error_pairs_h.keys())
            | set(self.distance_error_pairs_v.keys())
            | set(self.distance_error_pairs_epr.keys()))
        xs = []
        ys = []
        for d in distances:
            d_epr = self.distance_error_pairs_epr.get(d)
            if d_epr is not None:
                cells = int(math.ceil(self.rep.rounds / self.rep.code_distance))
                p = d_epr.logical_error_rate
                p = total_error_to_per_piece_error(p, cells)
                if p:
                    xs.append(d)
                    ys.append(p)
                continue
    ...
    def groups_from_data(data: MultiStats) -> Dict[float, 'LambdaGroup']:
        ...
        for key, val in data.data.items():
            ...
            g = groups[key.noise]
            if key.preserved_observable in "HX":
                d = g.distance_error_pairs_h
            elif key.preserved_observable in "VZ":
                d = g.distance_error_pairs_v
            elif key.preserved_observable == "EPR":
                d = g.distance_error_pairs_epr
            else:
                raise NotImplementedError(f'{key.preserved_observable=!r}')
